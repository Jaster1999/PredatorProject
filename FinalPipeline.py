...
def load_images_from_folder(folder):
    ...
    for filename in os.listdir(folder): #find all the images in the folder and save them to a list
        img = imread(os.path.join(folder,filename)) # opencv imread cant handle the .bmp file type, this will also load single channel images as single channel
        if img is not None:
            images.append(img)
    return images
    
def diff_mask(img1, img2, LWRthreshold, UPRthreshold, brightnessthreshold):
    avgPx = np.mean(img1) #measure the average value of the pixels in the image
    if avgPx > brightnessthreshold:
...
def main():
    CWD = os.getcwd()
    folderOfSeq = 'Video sequences for project-20210914'
    ...
    for seq in folders:
        ...
        for imagenumber in range(len(sequence)): # loop through all images in the sequence
            ...
            mask = diff_mask(BKground, img, LWRthresholdValue, UPRthresholdValue, 80)
            blur = cv.medianBlur(mask, 5)
            #plt.imshow(blur)
            #plt.show()
            cv.imshow("blurred mask", blur)
